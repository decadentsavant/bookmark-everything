...
// Installing runs Omarchy's own update command, which fast-forwards,
// validates, and reloads plugins.
Scope {
  id: service
  ...
  function handleCheck(text) {
    ...
    if (Local.HotkeyService.autoUpdate) { update(); return }
  ...
  function update() {
    ...
    Quickshell.execDetached(["sh", "-c", updateScript, "sh", pluginId, pluginDir, omarchyPath])
  }
  ...
  readonly property string updateScript: 'PATH="$3/bin:$PATH"; export PATH; ' +
    ...
    '  "$3/bin/omarchy-notification-send" "Bookmark Everything" "Updated to v${v:-?}"; ' +
    'else ' +
